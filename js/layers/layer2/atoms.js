addLayer("a", {
    ...
    nodeStyle() {   
        if(tmp.a.canReset || player.a.unlocked) {
            return {
        "background-image": "linear-gradient(90deg,rgb(46, 6, 6) 0%, rgb(50, 4, 4) 50%, rgb(97, 1, 1) 100%)",
    ...
    softcapPower() { 
        if(inChallenge('d', 11)) return new Decimal(0.01).add(player.d.points.mul(0.01))
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
        ...
        if(getBuyableAmount('ce', 18).gte(1)) mult = mult.times(buyableEffect("ce", 18))
        if(getBuyableAmount('ce', 41).gte(1)) mult = mult.times(buyableEffect("ce", 41))
        if(getBuyableAmount('ce', 101).gte(1)) mult = mult.times(buyableEffect("ce", 101))
        if(getBuyableAmount('ce', 108).gte(1)) mult = mult.times(buyableEffect("ce", 108))
        if(player.ce.points.gte(1)) mult = mult.times(new Decimal(4).mul(player.ce.points))
        ...
        if(inChallenge('t', 11)) {
            ...
            if(hasMilestone('h', 9)) mult = mult.times(player.v.points.plus(1).pow(0.5))
    ...
    gainExp() { // Calculate the exponent on main currency from bonuses
      ...
      passiveGeneration() {
        if(getBuyableAmount('t', 32).gte(1)) return new Decimal(0.02).mul(getBuyableAmount('t', 32))
    },
    doReset(reset){
        let keep = []
        keep.push("protons")
    ...
    12: {
        ...
        cost: new Decimal(3),
        effect(){   if(inChallenge('a', 21)) return new Decimal(1)
          else if(inChallenge("a", 11)) {return new Decimal(1)}
    ...
    24: {
        title: "Wrath",
        description: "Walk the plank, Proteins boost is increased from 1.2x to 1.7x",
...
challenges: {
    ...
    21: {
        ...
        challengeDescription: "Atom boosts, Cosmic Dust boosts, and any cell boosts no longer work. You are also given zero of all row 1 stats ",
        goalDescription: "Obtain 1e9 Multiplier",
        canComplete: function() {return player.m.points.gte(1e9)},
        unlocked() {
            if(inChallenge('a', 21) || (hasChallenge('a', 21))) return true
           else return (player.a.points.gte(300000))
        },
        completionLimit: 1,
        rewardDescription: "Cosmic Dust now boosts Atoms and unlocked row two upgrades in Atoms",
     ...
     22: {
        ...
        challengeDescription: "Combine challenge 12 and 21's effects(Deduction of life + Atominator) AND, a massive negative boost is applied to particle gain ",
        goalDescription: "Obtain 2e9 Quarks",
        canComplete: function() {return player.q.points.gte(2e9)},
        unlocked() {
            return (hasUpgrade('hm', 36))
        },
        completionLimit: 1,
        rewardDescription: "Keep all atom challenges on further resets plus, Tetra boosts particle gain.",
         countsAs: [["12"], ["21"]],
        onEnter() {
           player.a.stored.a = player.a.points
            player.a.stored.c = player.c.points
