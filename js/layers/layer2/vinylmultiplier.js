addLayer("v", {
    ...
    passiveGeneration() {
        if(getBuyableAmount('t', 31).gte(1)) return new Decimal(0.1).mul(getBuyableAmount('t', 31))
    },
    nodeStyle: {   
     "background-image": "linear-gradient(90deg,rgb(216, 255, 45) 0%, rgb(174, 199, 7) 50%, rgb(252, 249, 33) 100%)",
    ...
    gainExp() { // Calculate the exponent on main currency from bonuses
     ...
     doReset(reset){
        ...
        if(hasMilestone('hm', 2) && hasMilestone('ct', 1)) { keep.push("upgrades"); keep.push("buyables") }
    ...
    tabFormat: {
        "Main": {
            content: [
                "main-display",
                "prestige-button",
                ["display-text", function() { return "You have " + format(player.hm.points) + " hyper multiplier" }, {}],
                "buyables",
                "upgrades",
                ["toggle"   , function() { if(hasUpgrade('v', 12)) return ["h", "autoPrestige"] }]
            ] 
        }, 
    },
    upgrades: {
