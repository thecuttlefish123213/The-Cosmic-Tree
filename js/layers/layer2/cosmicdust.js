addLayer("c", {
    ...
    softcap() { 
            ...
            if(challengeCompletions('n', 11) >= 6 && inChallenge('n', 11)) return new Decimal(0.99)
            else return new Decimal(50) },
    softcapPower() { 
            let nissionite = player.n.points.toNumber()
            let equation = (0.01+(nissionite*0.01))/((challengeCompletions('n', 11)*2)+1)
            if(challengeCompletions('n', 11) >= 6 && inChallenge('n', 11)) equation = 0.001
            if(inChallenge('n', 11)) return new Decimal(equation)
            else return new Decimal(0.2) },
    doReset(reset){
        let keep = ["aChallengeUnlocked"]
        keep.push('t')
        keep.push('adt')
        keep.push('sat')
        keep.push("variable")
        ...
        if(hasMilestone('b', 1) && hasMilestone('ct', 2)) {keep.push("upgrades")
    ...
    color: "#0004ff",
    nodeStyle: {   
     "background-image": "linear-gradient(90deg,rgb(0, 4, 255) 0%, rgb(19, 1, 214) 50%, rgb(27, 11, 212) 100%)",
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
        ...
        if(getBuyableAmount('t', 24).gte(1)) mult = mult.times(buyableEffect('t', 24))
        if(getBuyableAmount('ce', 102).gte(1)) mult = mult.times(buyableEffect("ce", 102))
        if(player.chm.multiplicitiveInverse.gte(1)) mult = mult.times(player.chm.mInverseFunction())
     ...
     passiveGeneration() {
        if(getBuyableAmount('t', 33).gte(1)) return new Decimal(0.01).mul(getBuyableAmount('t', 33))
    ...
    row: 1, // Row the layer is in on the tree (0 is the first row)
     hotkeys: [
        {key: "c", description: "C: Reset for cosmic dust ", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
    branches: ["a", "q"],
    ...
    "Astronomy": {
        content: [
           ["display-text", function() {return 'You have ' + format(player.c.t) + ' telescopes.'},
            ...
            "blank",
            ["display-text", function() {if(hasChallenge('a', 11) ||  player.c.unlocked6.gte(1)) return 'You have ' + format(player.c.adt) + ' advanced telescopes.'},
                {"border-radius":"0px", "font-size": "28px"}
            ],
            "blank",
            
            ["display-text", function() {if(player.c.unlocked5.gte(1)) return 'You have ' + format(player.c.sat) + ' super advanced telescopes.'},
                {"border-radius":"0px", "font-size": "28px"}
            ],
            "blank",
            ["clickable", "11"],
...
clickables: {
    ...
    22: {
     ...
     25: {
        ...
        onClick() {
             if(player.c.adt >= 100 && player.c.t >= 100 && player.c.points >= 12 && player.q.sq >= 2000 && player.c.sat >= 50) {
    ...
    32: {
        ...
        onClick() {
            if(player.c.adt >= 150 && player.c.t >= 500 && player.c.points >= 10 && player.q.tq >= 999 && player.c.sat >= 75 ) {
