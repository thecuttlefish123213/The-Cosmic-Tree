 addLayer("b", {
    ...
    lore: {
        title: "Boracite",
        body() { return "Boracite is a rare Earth mineral, its atomic composition is MG3B7O13CL(those numbers are supposed to be subscripts). Boracite is the stand-in for the other path atoms could take, essentially all inorganic material. Cells, is the organic pathway." },
        
    },
     lore2: {
        title: "Geodes",
        body() { return `Geodes are an RNG buyable, where you have a random chance of getting some cool materials. They can either boost stats or be used in craftables! All geode stats survive resets(except explicit layer stats, like boracite)
           ` },
    ...
    startData() { return {
        ...
        vauqraseMultiplier() {
            return this.Vauqrase.lt(1) ? new Decimal(1) : new Decimal(1).add(this.Vauqrase.mul(40))
        },
        xenoomiumMultiplier() {
            return this.Xenoomium.lt(1) ? new Decimal(1) : new Decimal(400000).add(this.Xenoomium.mul(400000))
        },
        ...
        orbindiumMultiplier() {
           return this.Orbindium.lt(1) ? new Decimal(1) : new Decimal(3).add(this.Orbindium.mul(5))
        },
        bloodstoneMultiplier() {
            return this.Bloodstone.lt(1) ? new Decimal(1) : new Decimal(1000000).add(this.Bloodstone.mul(1000000))
        },
        ...
        koriumMultiplier() {
            return this.Korium.lt(1) ? new Decimal(1) : new Decimal(2).mul(this.Korium.mul(2))
        },
        kessiumMultiplier() {
            return this.Kessium.lt(1) ? new Decimal(1) : new Decimal(4).mul(this.Kessium.mul(16))
        },
    ...
    type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
     doReset(reset){
        let keep = ["Agate", "Moonstone", "Arbitorite", "Grandulum", "Voidestone"
            , "Orpiment", "Realgar", "Tigium", "Vauqrase", "Xenoomium",
            "Panenite", "Jahelium", "Trilithium", "Orbindium", "Bloodstone",
            "Kalium","Korium","Kessium","Megatronics"
        ]
    ...
    branches: ["t","c"],
    hotkeys: [
        {key: "b", description: "B: Reset for Boracite", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
    ...
    1: {
        ...
        effectDescription: "<b>BRR!!</b> It's cold. Passively generate Cosmic Dust at 100%, also passively generate telescopes. And keep cosmic dust upgrades on reset.",
    ...
    4: {
     ...
     5: {
        ...
        done() { return player.b.points.gte(150) },
        unlocked() { return challengeCompletions("n", 11) >= 3 },
    },
 },
     tabFormat: {
    "The Palisade Walls": {
        ...
        content: [
                ["infobox","lore2"],
                ["row",[["buyable", "11"],"blank",["v-line", "200px"], "blank", ["display-text", function() {return format(player.b.points) + " Boracite<br>" + format(player.b.Agate) + " Agate<br>" + format(player.b.Moonstone) 
                    + " Moonstone<br>" + format(player.b.Arbitorite) + " Arbitorite<br>" + format(player.b.Grandulum) + " Grandulum<br>" + format(player.b.Voidstone) + "<i> Voidstone</i>"
                }, {"font-size": "25px"}],"blank",["v-line", "200px"], "blank",["display-text", function() {return "Boosts:<br> " + "Boracite: Obvious<br>" + "Agate: None<br>" + "Moonstone: " + format(player.b.moonstoneMultiplier()) + "x, for Quarks<br>"
                    + "Arbitorite: " + format(player.b.arbitoriteMultiplier()) + "x, for Cosmic Dust<br>" + "Grandulum: None<br>" + "Void Stone: " + format(player.b.voidstoneMultiplier()) + "x, to Vinyl Multiplier"
                }]]], 
                "blank",
                ["h-line", "800px"],
                "blank",
                ["row", () => {return hasMilestone('b', 2) ? ([["buyable", "12"],"blank",["v-line", "200px"], "blank", 
                    ["display-text", 
                            format(player.t.points) + " Tetra<br>" 
                            + format(player.b.Orpiment) + " Orpiment<br>" 
                            + format(player.b.Tigium) + " Tigium<br>" 
                            + format(player.b.Realgar) + " Realgar<br>" 
                            + format(player.b.Vauqrase) + " Vauqrase<br>" 
                            + format(player.b.Xenoomium) + "<i> Xenoomium</i>", {"font-size": "25px"}
                        ],"blank",["v-line", "200px"], "blank",
                    ["display-text", 
                            "Boosts:<br> " + "Tetra: Obvious<br>" +
                            "Orpiment: None<br>" +
                            "Tigium: None<br>" 
                            + "Realgar: " + format(player.b.realgarMultiplier()) + "x, for Atoms<br>"
                            + "Vauqrase: " + format(player.b.vauqraseMultiplier()) + "x, for Mechanical Multiplier(and scrap)<br>" 
                            + "Xenoomium: " + format(player.b.xenoomiumMultiplier()) + "x, to Multiplier"
                ]]) : null}],
                "blank",
                ["h-line", "800px"],
                ["blank", "60px"],
                ["row", () => {return hasMilestone('b', 4) ? ([["buyable", "13"], ["blank", "60px"], ["blank", "60px"], 
                    ["display-text", 
                            format(player.chm.geometricDome) + " Geometric Domes<br>" 
                            + format(player.b.Panenite) + " Panenite<br>" 
                            + format(player.b.Jahelium) + " Jahelium<br>" 
                            + format(player.b.Trilithium) + " Trilithium<br>" 
                            + format(player.b.Orbindium) + " Orbindium<br>" 
                            + format(player.b.Bloodstone) + " <i>Bloodstone</i><br>", {"font-size": "25px"}
                        ],"blank",["v-line", "200px"], "blank",
                    ["display-text", 
                            "Boosts:<br> " + "Panenite: " + format(player.b.paneniteMultiplier()) + "x, for particles<br>" +
                            "Jahelium: " + "None<br>" +
                            "Trilithium: " +  "None<br>" +
                            "Orbindium: " + format(player.b.orbindiumMultiplier()) + "x, for Tetra<br>" +
                            "Bloodstone: " + format(player.b.bloodstoneMultiplier()) + "x, for Cosmic Dust"
                ]]) : null}],
                 ["blank", "60px"],
                ["h-line", "800px"],
                 "blank",
                ["row", () => {return hasMilestone('b', 5) ? ([["buyable", "14"],"blank",["v-line", "200px"], "blank", 
                    ["display-text", 
                            format(player.mm.points) + " Mega Multiplier<br>" 
                            + format(player.b.Kalium) + " Kalium<br>" 
                            + format(player.b.Korium) + " Korium<br>" 
                            + format(player.b.Kessium) + " Kessium<br>" 
                            + format(player.b.Megatronics) + " Megatronics<br>",
                           {"font-size": "21px"}
                        ],"blank",["v-line", "200px"], "blank",
                    ["display-text", 
                            "Boosts:<br> " + "Mega Multiplier: Obvious<br>" +
                            "Kalium: None<br>" +
                            "Korium: " + format(player.b.koriumMultiplier()) + "x, for Cosmic Dust<br>"
                            + "Kessium: " + format(player.b.kessiumMultiplier()) + "x, for Atoms<br>"
                            + "Megatronics: " + format(player.b.megatronicsMultiplier()) + "x, for Dimensional Points(Dimensions)<br>" 
    ...
    11: {
        ...
        display() { return "Spend Boracite for random materials! They can boost various stats or be used to craft craftables"
       ...
       12: {
         cost() { 
          ...
          title: "Tetra Geode",
        display() { return "Spend Tetra for random materials! They can boost various stats or be used to craft craftables"
            + " cost: " + format(this.cost()) 
    ...
    13: {
        ...
        display() { return "Spend Geometric Domes for random materials! They can boost various stats or be used to craft craftables"
            + " cost: " + format(this.cost()) 
           
         },
        
        canAfford() { return (player.chm.geometricDome.gte(this.cost())) },
        buyMax() {
            let costPerUnit = new Decimal(1)
            let max = new Decimal(1)
        ...
        buy() {
             let [max, costPerUnit] = this.buyMax()
            player.chm.geometricDome = player.chm.geometricDome.sub(costPerUnit.mul(max))
    ...
    14: {
         cost() { 
          ...
          title: "Mega Geode",
        display() { return "Spend Mega Multipliers for random materials! They can boost various stats or be used to craft craftables"
            + " cost: " + format(this.cost()) 
           
         },
        
        canAfford() { return (player.mm.points.gte(this.cost())) },
        buyMax() {
            let costPerUnit = new Decimal(1)
            let max = new Decimal(1)
