addLayer("q", {
    ...
    nodeStyle: {
       ...
       "width": "80px",
         "height": "120px",
        "background-image": "linear-gradient(90deg,rgb(195, 20, 195) 0%, rgb(254, 45, 254) 50%, rgb(132, 11, 113) 100%)",
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
        ...
        if(getBuyableAmount('ce', 104).gte(1)) mult = mult.times(buyableEffect("ce", 104))    
        if(player.b.Moonstone.gte(1)) mult = mult.times(player.b.moonstoneMultiplier())   
        ...
        if(buyableEffect('ce', 14)) mult = mult.times(buyableEffect('ce', 14))
         if (getBuyableAmount('t', 13).gte(1)) mult = mult.times(buyableEffect('t', 13))
        if (getBuyableAmount('t', 23).gte(1)) mult = mult.times(buyableEffect('t', 23))
        if(inChallenge('t', 11)) {
            mult = mult.times(1)
        } else {
    ...
    doReset(reset){
        ...
        keep.push("tq")
        keep.push("bq")
         if(hasUpgrade('hm', 35) && hasMilestone('ct', 1)) { keep.push("upgrades")}
   ...
   if(player.q.dq >= 1){
    ...
    "Upgrades and Prestige": {
        content: [
         ...
         ["upgrades", function() {return "upgrades", "2"}],],
    ...
    "Quark types": {
        ...
        ["infobox", "lore"],
            ...
            "blank",
             "blank",
            ["column",[["row",[["buyable", "11"],"blank",["buyable", "12"],"blank",["buyable", "13"]]]]],
            "blank",
            ["h-line", "700px"],
            "blank",
             ["column",[["row",[["buyable", "21"],"blank",["buyable", "22"],"blank",["buyable", "23"]]]]],
             "blank",
             ["h-line", "700px"],
            "blank",
...
buyables: {
    11: {
        ...
        display() { return "Exchange your Up Quarks for Down Quarks. Each quark boosts the gain of the quark below it" },
        canAfford() { return player.q.uq.gte(this.cost(1)) },
        buyMax() {if(hasUpgrade('q', 22)) return true},
        buy() {
    ...
    23: {
        ...
        display() { return "Exchange your Bottom Quarks for neutrons" },
    ...
    31: {
        ...
        title() { return "Divine Quark" },
        display() { return "1 googol bottom quarks(1e100) for 1 divine quark" },
        canAfford() { return player.q.bq.gte(this.cost(1)) },
       purchaseLimit: 1,
        buy() {
          player.q.bq = new Decimal(0)
    ...
    setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(1))
        ...
        style() {
            return {backgroundImage: "linear-gradient(90deg, rgba(255, 251, 0, 0.48) 0%, rgb(196, 196, 196) 50%, rgb(218, 215, 21) 100%)",
            borderRadius: "0px", 
            ...
            animation: "rippleMove 2s ease infinite, scalar 0.5s ease-in-out infinite",
             }
       },
