...
function load(saveString = null) {
  let get = saveString ?? localStorage.getItem(getModID());
  ...
  if (!Vue.isReactive(player)) player = Vue.reactive(player);
  if (!Vue.isReactive(options)) options = Vue.reactive(options);
  ...
  updateTemp();
  updateTabFormats();
  if (typeof syncVueStore === "function") syncVueStore();
  loadVue();
}
...
function runSave(button) {
  if (
    !confirm(
      "WARNING: This will reset all of your current progress, subcurrencies do not carry over, this is reccomended for experimentation or data loss only!",
    )
  ) {
    return null;
  } else importSave(button.dataset.answer);
}
